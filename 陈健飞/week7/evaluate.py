"""
在测试集上评估 BERT NER 模型，并统计非法序列（适配 peoples_daily 数据集）
...
BERT_PATH = "bert-base-chinese"  # 自动从 HuggingFace 下载
...
def main():
    ...
    # 使用 peoples_daily 的标签体系
    labels, label2id, id2label = build_label_schema(dataset_type="peoples_daily")
    ...
    _, val_loader, test_loader = build_dataloaders(
        ...
        data_dir=DATA_DIR,
        dataset_type="peoples_daily",  # 指定数据集类型
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="评估 BERT NER 模型（peoples_daily数据集）")
    parser.add_argument("--use_crf", action="store_true")
    parser.add_argument("--bert_path", type=str, default=BERT_PATH)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--split", choices=["validation", "test"], default="validation",
                        help="peoples_daily 可以使用 test 集评估")

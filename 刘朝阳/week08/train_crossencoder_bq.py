...
def main():
    ...
    # BQ Corpus 正负比约 30%:70%，CrossEntropyLoss 默认不加权
    # 如需加权，可参考 bert_text_classification 的 compute_class_weight 做法
    criterion = nn.CrossEntropyLoss()

    # ── 训练循环 ──────────────────────────────────────────────────────────
    ckpt_path   = CKPT_DIR / "bq_crossencoder_best.pt"
    best_val_f1 = 0.0
    log_records = []
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / "bq_crossencoder_log.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="CrossEncoder 训练（BQ Corpus 银行问句匹配）")
    ...
    parser.add_argument("--max_length",        default=192,  type=int,
                        help="句对总最大 token 数（两句拼接，BQ 部分句子较长建议 192）")

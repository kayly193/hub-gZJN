...
def main():
    ...
    # LCQMC 正负比约 50%:50%，类别较均衡，CrossEntropyLoss 默认不加权即可
    criterion = nn.CrossEntropyLoss()

    # ── 训练循环 ──────────────────────────────────────────────────────────
    ckpt_path   = CKPT_DIR / "lcqmc_crossencoder_best.pt"
    best_val_f1 = 0.0
    log_records = []
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / "lcqmc_crossencoder_log.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="CrossEncoder 训练（交互型文本匹配）—— LCQMC")
    ...
    parser.add_argument("--max_length",        default=192,  type=int,
                        help="句对总最大 token 数（LCQMC 句子偏长，建议 192）")

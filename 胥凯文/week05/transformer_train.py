...
class LM(nn.Module):
    ...
    def forward(self, x):
        # x: (B, T)
        e = self.embed(x) * math.sqrt(self.embed_dim)  # (B, T, D)
        e = self.pos_encoder(e.transpose(0, 1)).transpose(0, 1)  # (B, T, D)

        seq_len = x.size(1)
        mask = torch.triu(torch.ones(seq_len, seq_len) * float('-inf'), diagonal=1)
        mask = mask.to(x.device)
        
        out = self.transformer(e, mask=mask)  # (B, T, D)

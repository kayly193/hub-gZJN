...
def run(question: str, max_steps: int = 10, memory: AgentMemory | None = None) -> Generator[dict, None, None]:
    """
    ...
    if memory is None:
        memory = AgentMemory()
        memory.add_system(FC_SYSTEM_PROMPT)
    elif not any(msg.get("role") == "system" for msg in memory.get_messages()):
        memory.add_system(FC_SYSTEM_PROMPT)
    ...
    for step in range(1, max_steps + 1):
        ...
        for tool_call in msg.tool_calls:
            ...
            memory.add_tool(tool_call.id, str(observation))

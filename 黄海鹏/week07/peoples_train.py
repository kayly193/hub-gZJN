...
from peoples_dataset import build_label_schema, build_dataloaders
...
LOG_DIR = ROOT / "outputs" / "logs_peoples"

# 评估函数
def evaluate_epoch(
    model: nn.Module, # 模型
    loader,           # 数据加载器
    id2label: dict,   # id2label 字典
    device: torch.device, # 训练设备
    use_crf: bool,       # 是否用 CRF
) -> tuple[float, float]:
...
def train_one_epoch(
    ...
    total_loss = 0.0
    optimizer.zero_grad() # 每个 epoch 开始前清零梯度
    ...
    for step, batch in enumerate(pbar):
        ...
        (loss / grad_accum).backward() # 梯度累积：除以 grad_accum 平均损失，防止梯度过大 + 反向传播
        ...
        if (step + 1) % grad_accum == 0:
            ...
            optimizer.zero_grad() # 梯度累积后更新参数并清零
...
def main():
    ...
    # Tokenizer 作用 是将文本转换为 BERT 可接受的输入格式（input_ids, attention_mask, token_type_ids），并处理子词对齐。
    tokenizer = BertTokenizerFast.from_pretrained(str(args.bert_path))

    # DataLoader 作用 是封装数据集，提供批量加载、打乱、并行等功能，简化训练循环中的数据处理。
    ...
    # 通过参数分组实现分层学习率，BERT 层使用 args.lr，分类头使用 args.lr * args.head_lr_mult
    optimizer = AdamW(
        [
    ...
    print(f"\n下一步：python peoples_evaluate.py {'--use_crf' if args.use_crf else ''}")

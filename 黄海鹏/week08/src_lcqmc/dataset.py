...
class TripletDataset(Dataset):
    ...
    构建逻辑（离线静态构建）：
      ...
      → 最终 triplet 数量 ≈ 正样本对数量（LCQMC 约 10K 条）
...
# 1. 为 BiEncoder（CosineEmbeddingLoss / 评估）构建 train/val/test DataLoader。
def build_pair_loaders(data_dir, tokenizer, max_length=64, batch_size=32):
    ...
    注意：LCQMC test 集无正样本标签，实际评估用 val。
    """
    ...
    print(f"  test  : {len(test_ds):>7,} 条, {len(test_loader):>5} batch  (LCQMC test 无正样本，仅供参考)")
    return train_loader, val_loader, test_loader

# 2. 为 TripletLoss 训练构建 DataLoader，val/test 仍用 PairDataset。
...
# 3. 为 CrossEncoder 构建 train/val/test DataLoader。

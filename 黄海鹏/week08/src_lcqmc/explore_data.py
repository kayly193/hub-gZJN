...
# ── matplotlib 中文字体设置 ──────────────────────────────────────────────
# macOS 可用: PingFang SC, Heiti SC, STHeiti, Songti SC
# Windows 可用: SimHei, Microsoft YaHei, SimSun
# Linux 可用: Noto Sans CJK, WenQuanYi
...
def plot_label_distribution(splits_data, output_dir):
    ...
    fig.suptitle("LCQMC 标签分布（正例约 31%，负例约 69%）",
                 fontsize=12, y=1.02)
    fig.tight_layout()
...
# 解析命令行参数
def parse_args():
    parser = argparse.ArgumentParser(description="LCQMC 数据集探索")

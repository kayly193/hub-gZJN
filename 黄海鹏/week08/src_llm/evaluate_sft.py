...
MODEL_PATH  = ROOT.parent.parent / "pretrain_models" / "qwen" / "Qwen2-0.5B-Instruct"
    ...
    def read_log_f1(path, key="f1"):
        if Path(path).exists():
            ...
            if isinstance(data, list):
                return {
                    "accuracy": data[-1].get("val_acc", "?"),
                    "f1": data[-1].get("val_f1", "?")
                }
            return {
                "accuracy": data.get("accuracy", "?"),
                "f1": data.get("f1", data.get("metrics", {}).get("f1", "?"))
            }
        return {"accuracy": "?", "f1": "?"}
...
多方对比（LCQMC validation 集，所有方案均使用 Accuracy + F1，直接可比）
  ...
  │ 方法                                     │ Accuracy │ F1(pos)  │
  ├──────────────────────────────────────────┼──────────┼──────────┤
  │ BiEncoder + CosineEmbeddingLoss          │ {fmt(bi_cosine_f1['accuracy']):<8} │ {fmt(bi_cosine_f1['f1']):<8} │
  │ BiEncoder + TripletLoss                  │ {fmt(bi_triplet_f1['accuracy']):<8} │ {fmt(bi_triplet_f1['f1']):<8} │
  │ CrossEncoder + CrossEntropyLoss          │ {fmt(cross_f1['accuracy']):<8} │ {fmt(cross_f1['f1']):<8} │

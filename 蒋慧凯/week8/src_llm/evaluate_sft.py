...
def parse_args():
    ...
    parser.add_argument("--dataset",     default="afqmc",
...
def main():
    ...
    data_dir = Path(args.data_dir) if args.data_dir else ROOT / "data" / args.dataset
    ...
    # ── 加载数据 ──────────────────────────────────────────────────────────────
    data_path = data_dir / "validation.jsonl"
    ...
    for i, item in enumerate(samples, 1):
        ...
        status   = "[OK]" if pred == gold else ("[?]" if pred == -1 else "[X]")
    ...
    # 读取同数据集的 BiEncoder / CrossEncoder 日志（优先 ntrain 最大的）
    import re
    def find_log(prefix):
        pattern = f"{args.dataset}_{prefix}_*log.json"
        matches = sorted(LOG_DIR.glob(pattern),
                         key=lambda p: (int(re.search(r"ntrain(\d+)", p.name).group(1))
                                        if re.search(r"ntrain(\d+)", p.name) else 0),
                         reverse=True)
        return matches[0] if matches else None

    bi_cosine_f1  = read_log_f1(find_log("biencoder_cosine"))
    bi_triplet_f1 = read_log_f1(find_log("biencoder_triplet"))
    cross_f1      = read_log_f1(find_log("crossencoder"))
    ...
    print(f"""
多方对比（{args.dataset.upper()} validation 集，所有方案均使用 Accuracy + F1，直接可比）
  ┌──────────────────────────────────────────┬──────────┬──────────┐
  │ 方法                                     │ Accuracy │ F1(pos)  │
  ├──────────────────────────────────────────┼──────────┼──────────┤
...
""")
    ...
    # ── 保存结果 ──────────────────────────────────────────────────────────────
    out_path = LOG_DIR / f"{args.dataset}_sft_results.json"

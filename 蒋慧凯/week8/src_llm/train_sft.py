...
使用方式：
  python train_sft.py                        # LoRA，5000
  python train_sft.p条快速演示（默认）y --num_train -1         # LoRA，全部 34K 条
...
# 默认从 HuggingFace 下载 Qwen2-0.5B-Instruct；也可传入本地路径
MODEL_PATH = "Qwen/Qwen2-0.5B-Instruct"
OUTPUT_DIR = ROOT / "outputs"


def resolve_data_dir(args):
    """根据 --dataset 解析数据目录；若用户显式传了 --data_dir 则优先使用。"""
    if getattr(args, "data_dir", None):
        return Path(args.data_dir)
    return ROOT / "data" / args.dataset
...
def main():
    ...
    subset_tag = f"_ntrain{args.num_train}" if args.num_train > 0 else "_full"
    ckpt_dir   = output_dir / f"{args.dataset}_{'sft_full' if args.full_ft else 'sft_adapter'}{subset_tag}"
    ...
    print(f"训练集: {len(train_raw)} 条（正{n_each}+负{n_each}，平衡采样）| 验证集（前500条）: {min(500, len(val_raw))} 条")
    ...
    val_dataset   = SFTDataset(val_raw[:min(500, len(val_raw))], tokenizer, args.max_length)
    ...
    # ── 保存训练日志 ──────────────────────────────────────────────────────────
    log_tag  = f"{args.dataset}_{'full_ft' if args.full_ft else 'sft'}{subset_tag}"

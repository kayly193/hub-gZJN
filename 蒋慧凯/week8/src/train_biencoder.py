...
# 默认从 HuggingFace cache 加载 bert-base-chinese；也可传入本地路径
...
def main():
    ...
    print(f"\n数据集: {args.dataset}  目录: {data_dir}")
    if args.num_train:
        print(f"训练子集大小: {args.num_train}")
    tokenizer = BertTokenizer.from_pretrained(args.bert_path)
    print("DataLoader 构建中...")

    if args.loss == "cosine":
        train_loader, val_loader, _ = build_pair_loaders(
            data_dir, tokenizer,
            max_length=args.max_length, batch_size=args.batch_size,
            max_samples=args.num_train,
        )
    else:  # triplet
    ...
    # ── 训练循环 ──────────────────────────────────────────────────────────
    subset_tag = f"_ntrain{args.num_train}" if args.num_train else ""
    ckpt_name = f"{args.dataset}_biencoder_{args.loss}{subset_tag}_best.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_f1 > best_val_f1:
            ...
            print(f"  [OK] 新最优模型已保存 -> {ckpt_path}  (val_f1={val_f1:.4f})")

    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / f"{args.dataset}_biencoder_{args.loss}{subset_tag}_log.json"
...
def parse_args():
    ...
    parser.add_argument("--num_train",         default=None, type=int,
                        help="仅使用训练集前 N 条样本（小数据快速实验）")

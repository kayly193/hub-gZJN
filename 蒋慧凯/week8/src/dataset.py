...
class PairDataset(Dataset):
    ...
    def __init__(self, data_path_or_data, tokenizer, max_length=64):
        if isinstance(data_path_or_data, list):
            self.rows = data_path_or_data
        else:
            self.rows = load_jsonl(data_path_or_data)
...
class TripletDataset(Dataset):
    ...
    def __init__(self, data_path_or_data, tokenizer, max_length=64):
        ...
        self.max_length = max_length
        if isinstance(data_path_or_data, list):
            rows = data_path_or_data
        else:
            rows = load_jsonl(data_path_or_data)
    ...
    def _build_triplets(self, rows):
        ...
        for r in rows:
            ...
            if negs:
                ...
                # anchor 没有自身的负样本 → 从全局随机选（排除 anchor / positive）
                candidates = [s for s in global_pool if s not in (anchor, positive)]
                if not candidates:
                    continue  # 无法构造合法三元组，跳过
...
class CrossEncoderDataset(Dataset):
    ...
    def __init__(self, data_path_or_data, tokenizer, max_length=128):
        if isinstance(data_path_or_data, list):
            self.rows = data_path_or_data
        else:
            self.rows = load_jsonl(data_path_or_data)
...
def build_pair_loaders(data_dir, tokenizer, max_length=64, batch_size=32, max_samples=None):
    """
    为 BiEncoder（CosineEmbeddingLoss / 评估）构建 train/val/test DataLoader。
    注意：AFQMC test 集无正样本标签，实际评估用 val。
    max_samples: 训练集最大样本数，用于大数据集快速实验（None=全部）。
    """
    data_dir = Path(data_dir)
    train_rows = load_jsonl(data_dir / "train.jsonl")
    if max_samples is not None and max_samples < len(train_rows):
        train_rows = random.sample(train_rows, max_samples)
    train_ds = PairDataset(train_rows, tokenizer, max_length)
...
def build_triplet_loader(data_dir, tokenizer, max_length=64, batch_size=32, max_samples=None):
    ...
    if max_samples is not None and max_samples < len(train_rows):
        train_rows = random.sample(train_rows, max_samples)
    train_ds = TripletDataset(train_rows, tokenizer, max_length)
    val_ds   = PairDataset(data_dir / "validation.jsonl", tokenizer, max_length)
...
def build_crossencoder_loaders(data_dir, tokenizer, max_length=128, batch_size=32, max_samples=None):
    ...
    if max_samples is not None and max_samples < len(train_rows):
        train_rows = random.sample(train_rows, max_samples)
    train_ds = CrossEncoderDataset(train_rows, tokenizer, max_length)

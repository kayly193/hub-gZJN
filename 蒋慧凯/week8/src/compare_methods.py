...
使用方式：
  ...
  python compare_methods.py --dataset lcqmc --split validation --batch_size 64
...
def find_checkpoint(dataset, key):
    """根据数据集和方法 key 自动匹配 checkpoint 文件，优先选择 ntrain 最大的。"""
    pattern = f"{dataset}_{key}_*best.pt"
    matches = list(CKPT_DIR.glob(pattern))
    if not matches:
        return None
    # 按文件名中的 ntrain 数字降序，其次按文件修改时间降序
    def _sort_key(p):
        m = re.search(r"ntrain(\d+)", p.name)
        ntrain = int(m.group(1)) if m else 0
        return (ntrain, p.stat().st_mtime)
...
def load_and_eval(method, tokenizer, device, data_dir, split, batch_size):
    ckpt_path = find_checkpoint(method["dataset"], method["key"])
    if ckpt_path is None or not ckpt_path.exists():
        print(f"  [SKIP] checkpoint 不存在: {CKPT_DIR / method['ckpt_pattern']}")
        return None

    print(f"  [OK] 加载 checkpoint: {ckpt_path.name}")
...
def plot_comparison_bar(results, save_path):
    ...
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Score")
    dataset_name = save_path.stem.split("_")[0].upper()
...
def parse_args():
    ...
    parser.add_argument("--dataset",    default="afqmc", choices=["afqmc", "lcqmc", "bq_corpus"])
...
def main():
    ...
    print(f"设备: {device}  数据集: {args.dataset}  评估集: {args.split}")
    ...
    for m in METHODS:
        ...
        m["ckpt_pattern"] = f"{args.dataset}_{m['key']}_*best.pt"
        print(f"\n{'='*55}")
        print(f"加载 {m['key']} ...")
    ...
    # ── 控制台对比表 ──────────────────────────────────────────────────────
    header = f"{'方法':<30} {'Accuracy':>9} {'F1(weighted)':>13} {'AUC-ROC':>9} {'额外信息':>20}"
    print(f"\n{'='*85}")
    print(header)
    print(f"{'-'*85}")
    for m in all_results:
        auc_str = f"{m.get('auc', float('nan')):.4f}" if m["type"] == "biencoder" else "-"
        extra = (f"threshold={m['threshold']:.2f}"
                 if m["type"] == "biencoder" else "argmax")
        print(f"  {m['key']:<28} {m['accuracy']:>9.4f} {m['f1']:>13.4f} {auc_str:>9} {extra:>20}")
    ...
    if len(bi_results) == 2:
        ...
        if abs(delta_f1) < 0.01:
            print("    → 两种 Loss 差距不大（小数据 + 少量三元组限制了 Triplet 的优势）")
        elif delta_f1 > 0:
            print("    → TripletLoss 更优，三元组对语义距离的约束更精确")
        else:
            print("    → CosineEmbeddingLoss 更优，当前数据量下直接对标签优化更稳定")

    # ── 保存 Markdown 对比表 ──────────────────────────────────────────────
    md_lines = [f"## {args.dataset.upper()} 三种方法对比\n\n",
                "| 方法 | Accuracy | F1 (weighted) | Threshold | AUC-ROC |\n",
                "| --- | --- | --- | --- | --- |\n"]
    for m in all_results:
        if m["type"] == "biencoder":
            md_lines.append(
                f"| {m['key']} | {m['accuracy']:.4f} | {m['f1']:.4f} | {m['threshold']:.2f} | {m.get('auc', float('nan')):.4f} |\n"
            )
        else:
            md_lines.append(
                f"| {m['key']} | {m['accuracy']:.4f} | {m['f1']:.4f} | - | - |\n"
            )
    md_lines.append(f"\n- 柱状图: `figures\\{args.dataset}_acc_f1.png`\n")
    md_lines.append(f"- 相似度分布图: `figures\\{args.dataset}_sim_distribution.png`\n")
    ...
    log = [{k: _to_py(v) for k, v in m.items() if k not in SKIP_KEYS}
           for m in all_results]
    log_path = LOG_DIR / f"{args.dataset}_method_comparison.json"
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    with open(log_path, "w", encoding="utf-8") as f:
        json.dump(log, f, ensure_ascii=False, indent=2)
    print(f"对比日志 → {log_path}")

    # ── 可视化 ────────────────────────────────────────────────────────────
    plot_comparison_bar(all_results, FIG_DIR / f"{args.dataset}_acc_f1.png")

    bi_with_sim = [m for m in all_results if m["type"] == "biencoder" and "similarities" in m]
    if bi_with_sim:
        plot_sim_distributions(bi_with_sim, FIG_DIR / f"{args.dataset}_sim_distribution.png")

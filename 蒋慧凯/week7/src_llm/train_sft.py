...
适配 peoples_daily 数据集（PER / ORG / LOC）
...
def bio_to_target(tokens: list[str], ner_tags: list[str]) -> str:
    """将 BIO 标签序列转为 SFT 目标 JSON 字符串。"""
    entities = []
    i = 0
    n = len(tokens)
    while i < n:
...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = bio_to_target(item["tokens"], item["ner_tags"])
...
def main():
    ...
    if args.full_ft:
        ...
        if not PEFT_AVAILABLE:
            raise ImportError("LoRA 模式需要 peft 库")

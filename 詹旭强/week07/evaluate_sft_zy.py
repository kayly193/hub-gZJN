...
ADAPTER_DIR = ROOT / "outputs" / "sft_adapter_zy"
...
SYSTEM_PROMPT = (
    ...
    "实体类型（英文标识）：LOC（地址）、PER（人名）、ORG（公司）\n"
...
# ══════════════════════════════════════════════════════════════════════════════
# peoples_daily格式转化为格式 cluener
# ══════════════════════════════════════════════════════════════════════════════
def convert_bio_to_label_format(records: list[dict]) -> list[dict]:
    """
    将 BIO 标签格式的 NER 数据转换为包含 text 和 label 字典的格式。
    
    Args:
        records: 包含 tokens 和 ner_tags 的原始数据列表
        
    Returns:
        转换后的数据列表，每条记录包含 text 和 label 字段
    """
    converted_records = []
    ...
    for record in records:
        ...
        # 2. 解析 BIO 标签，提取实体
        # 结构: { "entity_type": { "entity_text": [[start, end], ...] } }
        label = {}
        ...
        while i < len(ner_tags):
            ...
            if isinstance(tag, str) and tag.startswith("B-"):
                entity_type = tag[2:]  # 提取实体类型，如 ORG, PER
                start_idx = i
                i += 1
                
                # 向后遍历，直到遇到非 I-xxx 标签为止
                while i < len(ner_tags) and isinstance(ner_tags[i], str) and ner_tags[i].startswith(f"I-{entity_type}"):
                    i += 1
                    
                # 计算结束索引（不包含）
                end_idx = i
                
                # 从 tokens 中提取实体文本并计算字符级跨度
                entity_tokens = tokens[start_idx:end_idx]
                entity_text = "".join(entity_tokens)
                
                # 计算字符级别的起止位置（基于拼接后的 text）
                ...
                if entity_type not in label:
                    label[entity_type] = {}
                if entity_text not in label[entity_type]:
                    label[entity_type][entity_text] = []
                    
                label[entity_type][entity_text].append([char_start, char_end])
            else:
                i += 1
...
def main():
    ...
    if not ckpt_dir.exists():
        ...
        print("  LoRA（默认）保存到:   outputs/sft_adapter_zy/")
        print("  全量微调保存到:        outputs/sft_full_ckpt_zy/")
        return
    ...
    #格式转换
    val_data = convert_bio_to_label_format(val_data)
    ...
    llm_zero_f1   = "?"
    llm_few_f1    = "?"
    crf_log_path  = LOG_DIR / "eval_crf_validation_zy.json"
    llm_log_path  = LOG_DIR / "eval_llm_zy.json"
...
""")
    ...
    # ── 保存结果 ──────────────────────────────────────────────────────────────
    out_path = LOG_DIR / "eval_sft_zy.json"

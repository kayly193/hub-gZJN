...
输出：
  output/figures/entity_distribution.png       各类实体频次分布
  output/figures/text_length_distribution.png  文本长度分布
  output/figures/entity_length_distribution.png 实体长度分布
"""
...
def _extract_entities(tokens: list[str], ner_tags: list[str]) -> list[tuple[str, str]]:
    """从 BIO 标签序列中提取实体列表，返回 [(实体类型, 实体文本), ...]"""
    entities = []
    current_type = None
    current_tokens = []
...
def collect_stats(records: list) -> dict:
    ...
    for row in records:
        ...
        entities = _extract_entities(tokens, ner_tags)

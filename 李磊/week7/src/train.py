...
def make_run_name(args) -> str:
    """根据关键超参数生成运行名称，不同参数不互相覆盖。"""
    tag = "crf" if args.use_crf else "linear"
    lr_str = f"{args.lr:.0e}".replace("e-0", "e-")
    return f"{tag}_lr{lr_str}_bs{args.batch_size}_ep{args.epochs}"
